...
var (
	...
	info     = Command{"info", "Get information about the in-house in this channel!", inhouseInfoCommand}
	join     = Command{"join", "Join the active in-house in this channel!", inhouseJoinCommand}
	generate = Command{"generate", "Generate a bracket for the current in-house in this channel!", inhouseGenerateCommand}
	unjoin   = Command{"unjoin", "Unjoin the active in-house in this channel!", inhouseUnjoinCommand}
)
...
func loadCommands() {
	...
	commMap[info.name] = info
	commMap[join.name] = join
	commMap[generate.name] = generate
	commMap[unjoin.name] = unjoin
}
